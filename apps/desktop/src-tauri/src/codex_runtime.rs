...
#[tauri::command]
pub async fn load_codex_runtime_feed(
  limit: Option<usize>,
) -> Result<CodexRuntimeFeed, String> {
...
fn build_descriptor_hints(cwd: &Path) -> Vec<String> {
  ...
  if cwd.join("src-tauri").join("tauri.conf.json").exists() || cwd.join("tauri.conf.json").exists() {
  ...
  for row in rows {
    let thread = CodexThreadRow {
      ...
      first_user_message: row
        .try_get("first_user_message")
        .map_err(|error| format!("Failed to read Codex thread first_user_message: {error}"))?,
      rollout_path: row
...
) -> (Vec<CodexRuntimeEventRecord>, Vec<CodexRuntimeFileChangeRecord>) {
  let Some(raw_rollout_path) = &thread.rollout_path else {
    return (create_thread_opened_event(thread).into_iter().collect(), Vec::new());
  };
      ...
      "response_item" => {
          ...
          parse_patch_record(thread, &item, &mut patch_counter)
        {
          ...
          for (index, file_change) in patch_changes.iter_mut().enumerate() {
            file_change.id = format!("file-change-{}-patch-{}-{}", thread.id, patch_counter, index);
...
fn create_thread_opened_event(thread: &CodexThreadRow) -> Option<CodexRuntimeEventRecord> {
  Some(CodexRuntimeEventRecord {
    ...
    summary: format!("Opened Codex thread: {}.", normalize_title(&thread.title, &thread.first_user_message)),
    metadata: json!({
      "cwd": thread.cwd,
...
fn summarize_text(value: &str, max_length: usize) -> String {
  ...
  format!("{}…", normalized.chars().take(max_length.saturating_sub(1)).collect::<String>())
}
